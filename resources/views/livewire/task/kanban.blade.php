<div class="flex flex-1 min-h-0 flex-col">
    <div x-data="{ draggingTaskId: null }" class="flex flex-1 min-h-0 gap-4 overflow-x-auto pb-4">
        @foreach ($board->columns as $column)
            <div
                class="flex w-64 flex-shrink-0 flex-col rounded-lg bg-gray-50 p-3 dark:bg-gray-900"
            ...
            >
                ...
                @if ($column->status === \App\Enums\TaskStatus::BACKLOG)
                    @can('create', \App\Models\Task::class)
                        <button type="button" wire:click="openCreate({{ $column->id }})" class="mt-2 w-full shrink-0 rounded-md border border-dashed border-gray-300 py-1 text-xs text-gray-500 hover:border-indigo-400 hover:text-indigo-600 dark:border-gray-600">
                            + Nova tarefa
                        </button>
                    @endcan
                @endif
