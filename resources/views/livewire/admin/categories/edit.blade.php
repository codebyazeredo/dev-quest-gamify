<x-modal title="Editar categoria">
    <form wire:submit="save" class="space-y-4">
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Pontos base</label>
            <input type="number" min="0" wire:model="base_points"
                class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100">
            @error('base_points') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
        </div>
